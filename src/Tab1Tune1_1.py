...
def TuningFun(tunef1, tunef2, maploc_tune, out_text, variable_peaktype, tab1, covlist, covlist_label, covlist_dict):
    ...
    if variable_peaktype.get() == 'Max1':
        ...
        for i in range(0, 1):
            ...
            # print tune result in output text box
            out_text.insert(END, tvar + '\n')
            # store tune value in entry box
            sp_str = sp_df3[i].Species[0].split("(")[0]
            tune_result_dict[sp_str] = str(round(bvar, 3))
        # print other
        for j in range(0, len(sp_df3[1])):  # 0-6 for v1, 0-7 for v2
            ...
            out_text.insert(END, tvar + '\n')
            # store tune value in entry box
            sp_str = sp_df3[j].Species[0].split("(")[0]
            tune_result_dict[sp_str] = str(round(bvar, 3))
    ...
    for i in covlist_label:
        # try:
        covlist_label[i].grid_forget()  # remove label from last run
    ...
    for i in range(len(covlist)):
        ...
        covlist_dict[covlist[i]] = ttk.Entry(tab1, width=10, state=NORMAL)
        covlist_dict[covlist[i]].grid(row=5 + i, column=5, columnspan=1, sticky='w', padx=5, pady=1)
